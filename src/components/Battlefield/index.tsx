...
export default function Battlefield() {
  ...
  return (
    <BattlefiledWrapper>
      <TeamWrapper team={blueTeam[0].team} hoverColor={hoverSourceHandler()} source={source?.team}>
        {blueTeam.map((player) => (
          <SmallCard key={player.id} player={player} />
        ))}
      </TeamWrapper>
      <Divider />
      <TeamWrapper team={redTeam[0].team} hoverColor={hoverSourceHandler()} source={source?.team}>
        {redTeam.map((player) => (
          <SmallCard key={player.id} player={player} />
        ))}
      </TeamWrapper>
