...
export function setOrder(allTeam: Array<Unit>):Array<number>{
    return [...allTeam].sort((unit, nextUnit)=> {
...
export function nextSource(players: Array<Unit>, order: Array<number>, sourceNum: number):NextSourceReturn{
    ...
    while (i < 12){
        ...
        if(players[order[i]].HP !== 0){
            if(players[order[i]].paralyzed)
                players[order[i]].paralyzed = false
            else
